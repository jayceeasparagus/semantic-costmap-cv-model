...
def test_project_optical_points_uses_ros_axes():
    matrix = np.array(
        [[100.0, 0.0, 50.0], [0.0, 120.0, 40.0], [0.0, 0.0, 1.0]]
    )
    points = np.array([[1.0, 2.0, 10.0], [-1.0, -2.0, 10.0]])
    result = project_optical_points(points, matrix, (100, 80))
    np.testing.assert_allclose(result.columns, [60.0, 40.0])
    np.testing.assert_allclose(result.rows, [64.0, 16.0])
